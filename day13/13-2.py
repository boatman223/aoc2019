...
def print_image():
    xvalues = [i[0] for i in tiles.keys()]
    yvalues = [i[1] for i in tiles.keys()]
    for i in range(min(yvalues), max(yvalues)+1):
        line = ''
        for j in range(min(xvalues), max(xvalues)+1):
            if (j, i) == (-1, 0): line += f'{str(tiles[(j,i)])}\n'
            elif j == -1: continue
            elif tiles[(j,i)] == 0: line += '  '
            elif tiles[(j,i)] == 1: line += '..'
            elif tiles[(j,i)] == 2: line += '[]'
            elif tiles[(j,i)] == 3: line += "=="
            elif tiles[(j,i)] == 4: line += '()'
        print(line)

def timestep(ball, paddle):
    output = game.get_output()
    for i in range(0, len(output), 3):
        if output[i+2] == 3: paddle = output[i]
        if output[i+2] == 4: ball = output[i]
...
game = Computer(basecode[:])
ball = 0
paddle = 0
steps = 0
while game.status != game.HALTED:
    steps += 1
    ball, paddle = timestep(ball, paddle)
    if ball > paddle:
        game.send_input(1)
    elif paddle > ball:
        game.send_input(-1)
        ...
        game.send_input(0)
    if steps % 50 == 0: print_image()

timestep(ball, paddle)
print_image()